...
async function loadDash(role, page) {
  ...
  try {
    ...
    let map = {
      ...
      'crop lots': '/lots', 'product listings': '/lots', offers: '/offers', orders: '/orders', equipment: '/equipment', schemes: '/schemes',
...
async function recycleBin(el) {
  ...
  let groups = [
    { key: 'farmer', label: 'Farmer accounts', test: x => x.type === 'User' && x.role === 'farmer' },
    { key: 'fpo', label: 'FPO accounts', test: x => x.type === 'User' && x.role === 'fpo' },
    { key: 'buyer', label: 'Buyer accounts', test: x => x.type === 'User' && x.role === 'buyer' },
    { key: 'lots', label: 'Crop products', test: x => x.type === 'Lot' },
    { key: 'tools', label: 'Tools', test: x => x.type === 'Equipment' },
    { key: 'offers', label: 'Offers', test: x => x.type === 'Offer' },
    { key: 'orders', label: 'Orders', test: x => x.type === 'Order' },
    { key: 'other', label: 'Other deleted records', test: x => !['User', 'Lot', 'Equipment', 'Offer', 'Order'].includes(x.type) }
  ];
  ...
  let renderRows = group => {
    ...
    if (!groupRows.length) { panel.innerHTML = `<div class="empty">${t('No deleted records in this section.')}</div>`; return }
    ...
    panel.innerHTML = `<div class="toolbar recycle-toolbar"><label class="select-toggle"><input id="recycleAll" type="checkbox"> ${t('Select all')}</label><button class="button" id="restoreSelected" type="button">${t('Retrieve selected')}</button></div><div class="table-scroll"><table><thead><tr><th></th>${keys.map(key => `<th>${key}</th>`).join('')}<th>Action</th></tr></thead><tbody>${groupRows.map(row => `<tr data-id="${html(row._id)}" data-type="${html(row.type)}"><td><input class="recyclePick" type="checkbox"></td>${keys.map(key => `<td>${key.includes('status') ? `<span class="badge ${html(row[key])}">${html(row[key])}</span>` : html(row[key])}</td>`).join('')}<td><button class="ghost retrieveOne" type="button">${t('Retrieve')}</button></td></tr>`).join('')}</tbody></table></div>`;
  ...
  el.innerHTML = `<div class="recycle-tabs">${groups.map(group => `<button class="ghost recycle-tab" data-group="${group.key}" type="button">${t(group.label)} <span class="tab-count">${rows.filter(group.test).length}</span></button>`).join('')}</div><div id="recycleRows"></div>`;
...
function table(el, rows, page, role) {
  ...
  let hasBulkPick = isUserMgmt || isLotMgmt || isEquipmentMgmt;
  let actionCell = x => isUserMgmt
    ...
    : isOfferMgmt ? `<select class="offerStatus">${OFFER_STATUSES.map(s => `<option ${s === x.status ? 'selected' : ''}>${s}</option>`).join('')}</select> <button class="ghost updateOffer">${t('Update')}</button>`
  ...
  $('#bulkDelete') && ($('#bulkDelete').onclick = () => {
    ...
    buttonTask($('#bulkDelete'), async () => {
      ...
      else await Promise.all(ids.map(id => api((isLotMgmt ? '/lots/' : '/equipment/') + id, { method: 'DELETE' })));
