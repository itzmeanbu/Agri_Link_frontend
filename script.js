...
document.addEventListener('DOMContentLoaded',()=>{document.querySelectorAll('.brand-mark').forEach(el=>{const img=document.createElement('img');img.src='logo.jpg';img.alt='AgriLink logo';img.className='brand-logo';el.replaceWith(img)});document.body.innerHTML=document.body.innerHTML.replace(/mandi/gi,'regional')});
