...
function formModal(type) {
  ...
  let m = document.createElement('div'); m.className = 'modal';
  m.innerHTML = `<div><h2>${t('Add')} ${tPage(type)}</h2><form id="form">${fields.map(f => `<div class="field"><label>${tf(f)}<input name="${f}" type="${numFields.includes(f) ? 'number' : dateFields.includes(f) ? 'date' : 'text'}" ${reqFields.includes(f) ? 'required' : ''}></label></div>`).join('')}${withPhotos ? `<div class="field"><label>${imageLabel} <small class="muted">(up to 4)</small></label><div id="photoPreview" class="photo-preview"></div><input id="photoInput" type="file" accept="image/*" multiple></div>` : ''}<button class="button">${t('Save')}</button> <button type="button" class="ghost" id="cancel">${t('Cancel')}</button></form></div>`;
  document.body.append(m);
  $('#cancel').onclick = () => m.remove();
